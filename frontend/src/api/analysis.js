...
function common({departmentCode='',shops=[],warehouses=[],productSearch='',includeName='',excludeName='',productCodes=[],productCategoryIds=[]}={}){return {department_code:currentDepartmentCode(departmentCode),shops,warehouses,product_search:productSearch,include_name:includeName,exclude_name:excludeName,product_codes:productCodes,product_category_ids:productCategoryIds}}
export const fetchInventoryAnalysis=(options={})=>apiRequest('/api/analysis/inventory',{params:{...common(options),days:options.days||30,category:options.category||undefined,detail_warehouses:options.detailWarehouses||undefined,detail_product_categories:options.detailProductCategories||undefined}})
...
export const fetchProductCategories=({departmentCode=''}={})=>apiRequest(
  '/api/analysis/product-categories',
  {
    params:{department_code:currentDepartmentCode(departmentCode)}
  }
)
