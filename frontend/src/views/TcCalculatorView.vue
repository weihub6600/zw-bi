...
function onDocumentPointerDown(e){if(!e.target.closest('.combo'))closeAllCombos();}
function bind(){for(const id of['op','oc','dp','dc'])bindCombo(id);document.addEventListener('pointerdown',onDocumentPointerDown,true);for(const id of['modeEstimate','modeReview'])$(id).addEventListener('change',updateMode);$('season').addEventListener('change',()=>{$('seasonBadge').textContent=$('season').value==='peak'?'旺季价格':'淡季价格';});$('pickup').addEventListener('change',()=>{$('pickupText').textContent=$('pickup').checked?'是，上门提货':'否，自送到仓';});$('transportMin').addEventListener('change',()=>{if(!$('transportMin').disabled){quickTransportMin=$('transportMin').checked;$('transportMinText').textContent=quickTransportMin?'是，按线路最低收费':'否，不按最低收费';}});$('calcBtn').addEventListener('click',calculate);$('resetBtn').addEventListener('click',reset);$('copyBtn').addEventListener('click',copyResult);$('printBtn').addEventListener('click',()=>window.print());}
