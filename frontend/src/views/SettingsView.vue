...
function formatDateTime(v){
  ...
  if(m)return `${m[1]}-${String(m[2]).padStart(2,'0')}-${String(m[3]).padStart(2,'0')} ${String(m[4]).padStart(2,'0')}:${m[5]}`
  return raw.replace('T',' ').replace(/\.\d+(Z|[+-]\d{2}:?\d{2})?$/,'').slice(0,16)
}
...
<div class="page settings-page">
  ...
  <template v-if="tab==='activity'">
    ...
    <section class="panel settings-table-wrap"><div class="section-head"><h3>用户活跃明细</h3><button class="settings-mini" @click="loadActivity"><RefreshCw :size="14"/>刷新</button></div><table><thead><tr><th>用户</th><th>角色</th><th>最近登录</th><th>最近活跃</th><th>状态</th><th>今日操作</th><th>7天操作</th><th>未完成待办</th></tr></thead><tbody><tr v-for="u in activity.items||[]" :key="u.user_id"><td><strong>{{u.username}}</strong><small>{{u.user_id}}</small></td><td>{{roleText(u.role)}}</td><td><time :datetime="u.last_login_time||undefined">{{formatDateTime(u.last_login_time)}}</time></td><td><time :datetime="u.last_active_time||undefined">{{formatDateTime(u.last_active_time)}}</time></td><td><span :class="['presence',u.presence]">{{presence(u.presence)}}</span></td><td>{{u.today_ops}}</td><td>{{u.week_ops}}</td><td>{{u.incomplete_tasks}}</td></tr></tbody></table></section>
  </template>
  ...
  <template v-if="tab==='audit'">
    ...
    <section class="panel settings-table-wrap"><table><thead><tr><th>时间</th><th>用户</th><th>部门</th><th>动作</th><th>详情</th></tr></thead><tbody><tr v-for="x in audit.items||[]" :key="x.id"><td><time :datetime="x.created_at||undefined">{{formatDateTime(x.created_at)}}</time></td><td><strong>{{x.username}}</strong><small>{{x.user_id}}</small></td><td>{{x.department_name||'系统级'}}</td><td><code>{{x.action_type}}</code></td><td class="audit-detail">{{detailText(x.action_detail)}}</td></tr><tr v-if="!(audit.items||[]).length"><td colspan="5" class="dc-empty">暂无审计记录</td></tr></tbody></table></section>
    <section v-if="canGrantAdmin" class="panel settings-table-wrap audit-login-panel"><div class="section-head"><div><h3>登录安全审计</h3><span>仅系统管理员可见，包含成功/失败登录和来源 IP</span></div></div><table><thead><tr><th>时间</th><th>登录标识</th><th>识别用户</th><th>结果</th><th>IP</th><th>User-Agent</th></tr></thead><tbody><tr v-for="x in audit.login_items||[]" :key="'login-'+x.id"><td><time :datetime="x.login_time||undefined">{{formatDateTime(x.login_time)}}</time></td><td>{{x.login_key}}</td><td>{{x.username||'未识别'}}<small>{{x.user_id||'—'}}</small></td><td><span :class="['presence',x.result==='success'?'online':'inactive']">{{x.result==='success'?'成功':'失败'}}</span></td><td>{{x.ip_address||'—'}}</td><td class="audit-detail">{{x.user_agent||'—'}}</td></tr></tbody></table></section>
  </template>
