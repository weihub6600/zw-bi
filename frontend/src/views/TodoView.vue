...
import { createTask, decideTaskDelete, fetchTaskAssignees, fetchTaskOptions, fetchTasks, requestTaskDelete, saveTaskOwnerNote, withdrawTaskDelete } from '../api/tasks'
...
const assignees=ref([]),options=ref({shops:[],warehouses:[]}),createOpen=ref(false),createError=ref(''),creating=ref(false),optionsError=ref('')
const form=reactive({merchant_code:'',owner_user_id:'',target_qty:'',assign_date:localToday(),manager_note:'',shop_ids:[],warehouse_ids:[]})
let timer=null
...
async function loadOptions(){optionsError.value='';try{const [a,o]=await Promise.all([fetchTaskAssignees(),fetchTaskOptions()]);assignees.value=a.rows||[];options.value={shops:o.shops||[],warehouses:o.warehouses||[]};if(!form.owner_user_id&&assignees.value.length)form.owner_user_id=assignees.value[0].user_id}catch(e){optionsError.value=e.message||'店铺/仓库选项加载失败';error.value=e.message}}
function openCreate(sku=''){form.merchant_code=sku||'';if(!form.owner_user_id&&assignees.value.length)form.owner_user_id=assignees.value[0].user_id;form.target_qty='';form.assign_date=localToday();form.manager_note='';form.shop_ids=[];form.warehouse_ids=[];createError.value='';createOpen.value=true}
async function submitCreate(){creating.value=true;createError.value='';try{await createTask({...form,target_qty:form.target_qty===''?null:Number(form.target_qty),shop_ids:form.shop_ids,warehouse_ids:form.warehouse_ids});createOpen.value=false;tab.value='mine';await load()}catch(e){createError.value=e.message}finally{creating.value=false}}
// 任务店铺/仓库紧凑显示：前2个 + “+N”
function dimSummary(items){const names=(items||[]).map(x=>x.name||'').filter(Boolean);if(!names.length)return '';return names.length<=2?names.join('、'):`${names.slice(0,2).join('、')} +${names.length-2}`}
...
onMounted(async()=>{await loadOptions();if(route.query.create==='1'){openCreate(String(route.query.sku||''))}await load()})
</script>
...
<template>
  ...
  <SearchableMultiSelect v-model="form.shop_ids" :options="options.shops" placeholder="请选择店铺" search-placeholder="搜索店铺" />
  <div v-if="optionsError" class="ms-empty warn">{{ optionsError }} <a @click="loadOptions">重试</a></div>
</label>
<label>仓库（可多选）
  <SearchableMultiSelect v-model="form.warehouse_ids" :options="options.warehouses" placeholder="请选择仓库" search-placeholder="搜索仓库" />
  <div v-if="optionsError" class="ms-empty warn">{{ optionsError }} <a @click="loadOptions">重试</a></div>
</label><label class="full">管理员任务要求<textarea v-model="form.manager_note" placeholder="任务要求创建后负责人不能修改"></textarea></label></div><div v-if="createError" class="api-error compact">{{ createError }}</div><div class="modal-actions"><button class="rank-tab" @click="createOpen=false">取消</button><button class="rank-tab active" :disabled="creating" @click="submitCreate">{{ creating?'创建中…':'创建待办' }}</button></div></div></div>
  </div>
</template>

<style scoped>
.task-create-modal { max-height: none !important; overflow: visible !important; }
.task-create-modal .preset-form-grid > label { min-width: 0; }
.ms-empty { padding: 8px; text-align: left; color: #94a3b8; font-size: 12px; }
.ms-empty.warn { color: #dc2626; }
.ms-empty.warn a { color: #2563eb; cursor: pointer; text-decoration: underline; }
</style>
