...
async function loadOptions(){
  try{
    const r=await fetchDashboardOptions();shopOptions.value=r.shops||[];warehouseOptions.value=r.warehouses||[]
        ...
        <table class="inventory-table">
          ...
          <tbody>
            <tr v-for="r in rows" :key="r.product_id">
              ...
              <td>{{ r.sku }}</td><td><span :class="['stock-category-tag',`stock-${r.category}`]">{{ r.category_label }}</span></td>
            ...
            <tr v-if="!rows.length"><td colspan="10" class="table-empty">当前筛选条件下没有库存商品。</td></tr>
          </tbody>
        </table>
      </section>
    </template>
